 ...
 * @return {Promise<void>}
 */
export async function handleGetBillComparison(
 ...
 * @return {Promise<void>}
 */
export async function handleGetCheapestShopsForItem(
  req: Request, res: Response): Promise<void> {
  try {
    ...
    const shopsSnap = await db.collection("shops")
      .where("location", ">=",
        new admin.firestore.GeoPoint(bounds.minLat, bounds.minLng))
      .where("location", "<=",
        new admin.firestore.GeoPoint(bounds.maxLat, bounds.maxLng))
      .limit(100)
      .get();
    ...
    for (const shopDoc of shopsSnap.docs) {
      ...
      if (shopLoc.longitude < bounds.minLng ||
          shopLoc.longitude > bounds.maxLng) continue;
 ...
 * @return {Promise<void>}
 */
export async function handleGetSimilarProducts(
  req: Request, res: Response): Promise<void> {
  try {
    await verifyAuth(req);
    const normalizedName = req.query.normalizedName as string;
